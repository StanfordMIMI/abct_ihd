...
def bootStrapModelAUPRC(y_true, y_pred, numBootstraps=2000, stratified=True):
    """
    Returns upper and lower 95% confidence intervals for a model by bootstrapping the true and predicted
    labels for a model.
    """
    ...
    if stratified == True:
        pos_label_idx = np.argwhere(y_true==1).squeeze()
        npos = len(pos_label_idx)
        neg_label_idx = np.argwhere(y_true==0).squeeze()
        nneg = len(neg_label_idx)
    else:
        n=len(y_true)
        labels_idx = np.arange(0,len(y_true))

    def bootstrap_iter(labels, preds):
        if stratified==True:
            sample_idxs = np.concatenate((np.random.choice(pos_label_idx, replace=True, size=npos), \
                                          np.random.choice(neg_label_idx, replace=True, size=nneg)))
        else:
            sample_idxs = np.random.choice(labels_idx, size=n, replace=True)
        sample_labels = labels[sample_idxs]
        sample_preds = preds[sample_idxs]
        
        return average_precision_score(sample_labels, sample_preds)

    # Perform iterations and get values
    bootstrapAUPRCs = [bootstrap_iter(y_true, y_pred) for _ in range(numBootstraps)]
...
def bootstrapPRCDiff(labels, preds1, preds2, n_bootstraps=2000, stratified=True):
    n = len(labels)

    # Get original AUPRC
    auc1 = average_precision_score(labels, preds1)
    auc2 = average_precision_score(labels, preds2)
   
    pos_label_idx = np.argwhere(labels==1).squeeze()
    npos = len(pos_label_idx)
    neg_label_idx = np.argwhere(labels==0).squeeze()
    nneg = len(neg_label_idx)
    labels_idx = np.arange(0,len(labels))
    
    def bootstrap_iter(labels, preds1, preds2):
        if stratified==True:
            sample_idxs = np.concatenate((np.random.choice(pos_label_idx, replace=True, size=npos), \
                                          np.random.choice(neg_label_idx, replace=True, size=nneg)))
        else:
            sample_idxs = np.random.choice(labels_idx, size=n, replace=True)
        sample_labels = labels[sample_idxs]
        sample_preds1 = preds1[sample_idxs]
        sample_preds2 = preds2[sample_idxs]
        
        return average_precision_score(sample_labels, sample_preds1)-average_precision_score(sample_labels, sample_preds2)
    diffs = [bootstrap_iter(labels, preds1, preds2) for _ in range(n_bootstraps)]
    
    # Calculate statistic
    D = (auc1-auc2)/np.std(diffs)
    
    pval = 2*norm.cdf(-np.abs(D)) #assumes 2 sided test

    
    return pval

def formatPValue(p_value):
    """
    Formats P value as a string for printing
    """
    if p_value < 0.001:
        return "<0.001"
    elif p_value<0.01 or (f"{p_value:.2f}"=="0.05" and p_value<0.05):
        return f"{p_value:.3f}"
    else:
        return f"{p_value:.2f}"